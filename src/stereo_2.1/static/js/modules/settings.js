...
function getDisparityMethod() {
    fetch('/api/disparity/method')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // Update UI with current method
                document.getElementById('disparity-method').value = data.current_method;

                // Update DL status display
                const dlStatus = document.getElementById('dl-status');
                if (data.dl_available) {
                    dlStatus.innerHTML = `<span class="badge bg-success">Deep Learning Available (${data.dl_model_name})</span>`;
                    document.getElementById('download-dl-model-btn').disabled = true;
                } else {
                    dlStatus.innerHTML = `<span class="badge bg-warning">Deep Learning Model Not Loaded</span>`;
                    document.getElementById('download-dl-model-btn').disabled = false;
                }

                // Show/hide DL settings based on selected method
                if (data.current_method === 'dl') {
                    document.getElementById('dl-settings').style.display = 'flex';
                } else {
                    document.getElementById('dl-settings').style.display = 'none';
                }
            } else {
                showToast('Error', 'Failed to get disparity method', 'danger');
            }
        })
        .catch(error => {
            console.error('Error getting disparity method:', error);
            showToast('Error', 'Failed to get disparity method', 'danger');
        });
}

// Function to get DL parameters
function getDLParams() {
    fetch('/api/disparity/dl_params')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // Update UI with current parameters
                document.getElementById('dl-downscale').value = data.dl_params.downscale_factor;
                document.getElementById('dl-precision').value = data.dl_params.mixed_precision.toString();
            } else {
                showToast('Error', 'Failed to get DL parameters', 'danger');
            }
        })
        .catch(error => {
            console.error('Error getting DL parameters:', error);
            showToast('Error', 'Failed to get DL parameters', 'danger');
        });
}

// Function to update disparity method
function updateDisparityMethod() {
    const method = document.getElementById('disparity-method').value;
    const showConfidence = document.getElementById('show-confidence').checked;

    // Show saving indicator
    const saveBtn = document.getElementById('update-disparity-method-btn');
    const originalText = saveBtn.innerHTML;
    saveBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span> Saving...';
    saveBtn.disabled = true;
    ...
    .then(data => {
        if (data.success) {
            // If method is DL, also update DL parameters
            if (method === 'dl') {
                const downscaleFactor = parseFloat(document.getElementById('dl-downscale').value);
                const mixedPrecision = document.getElementById('dl-precision').value === 'true';
                ...
                .then(dlData => {
                    if (dlData.success) {
                        ...
                        showToast('Error', dlData.message, 'danger');
                        finalizeUpdate(false);
                    }
                })
                .catch(error => {
                    console.error('Error updating DL parameters:', error);
                    showToast('Error', 'Failed to update DL parameters', 'danger');
                    finalizeUpdate(false);
                });
            } else {
                finalizeUpdate(true);
            ...
            // Update config with show_confidence flag
            fetch('/api/config', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    .catch(error => {
        console.error('Error updating disparity method:', error);
        showToast('Error', 'Failed to update disparity method', 'danger');
        finalizeUpdate(false);
    });

    // Helper function to finalize the update
    function finalizeUpdate(success) {
        // Restore button
        saveBtn.innerHTML = originalText;
        saveBtn.disabled = false;
        ...
        if (success) {
            ...
            // Update visibility of DL settings
            if (method === 'dl') {
                document.getElementById('dl-settings').style.display = 'flex';
            } else {
                document.getElementById('dl-settings').style.display = 'none';
...
// Function to download deep learning model
function downloadDLModel() {
    // Show downloading indicator
    const dlBtn = document.getElementById('download-dl-model-btn');
    const originalText = dlBtn.innerHTML;
    dlBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span> Downloading...';
    dlBtn.disabled = true;
    ...
    .then(data => {
        if (data.success) {
            showToast('Success', 'Deep learning model downloaded and initialized', 'success');
            addToActivityLog('Deep learning model initialized');
    ...
    .catch(error => {
        ...
        showToast('Error', 'Failed to download model', 'danger');
...
function initSettings() {
    ...
    // Add new disparity method event listeners
    document.getElementById('update-disparity-method-btn').addEventListener('click', updateDisparityMethod);
    document.getElementById('download-dl-model-btn').addEventListener('click', downloadDLModel);

    // Handle method change to show/hide DL settings
    document.getElementById('disparity-method').addEventListener('change', function() {
        if (this.value === 'dl') {
            document.getElementById('dl-settings').style.display = 'flex';
        } else {
            document.getElementById('dl-settings').style.display = 'none';
    ...
    document.getElementById('settings-tab').addEventListener('shown.bs.tab', function() {
        loadCurrentSettings();
        getDisparityMethod();  // Load disparity method and DL status
        getDLParams();         // Load DL parameters
    });
}
